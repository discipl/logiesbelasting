...
static void digest_single_chunk(signed char *key_fragment,
...
static void init_shas(const signed char *seed_bytes, uint32_t idx,
                      cx_sha3_t *key_sha, cx_sha3_t *digest_sha)
{
    // use temp bigint so seed not destroyed
    signed char bytes[48];
...
void get_public_addr( signed char *seed_bytes, uint32_t idx,
                     uint8_t security, signed char *address_bytes)
{
    ...
    signed char digest[48 * security];
